        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-[#161615] overflow-hidden shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg">
                <div class="p-6 text-gray-900 dark:text-[#EDEDEC]">
                    <h3 class="text-lg font-medium mb-4">{{ __('Airline Details') }}</h3>
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <dt class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ __('Code') }}</dt>
                            <dd class="mt-1 text-sm">{{ $airline->code }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ __('Name') }}</dt>
                            <dd class="mt-1 text-sm">{{ $airline->name }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ __('Created At') }}</dt>
                            <dd class="mt-1 text-sm">{{ $airline->created_at->format('Y-m-d H:i:s') }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ __('Last Updated') }}</dt>
                            <dd class="mt-1 text-sm">{{ $airline->updated_at->format('Y-m-d H:i:s') }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-[#161615] overflow-hidden shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg">
                <div class="p-6 text-gray-900 dark:text-[#EDEDEC]">
                    ...
                    @if($airline->agentSales->count() > 0)
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                                <thead class="bg-[#FDFDFC] dark:bg-[#161615]">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">PNR</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">Passenger</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">Amount</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">Time Limit</th>
                                ...
                                <tbody class="bg-white dark:bg-[#161615] divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                                    @foreach($airline->agentSales as $sale)
                                        <tr>
                    ...
                    @else
                        <p class="text-[#706f6c] dark:text-[#A1A09A] text-sm">{{ __('No agent sales found for this airline.') }}</p>
                    @endif
                </div>
            </div>
        </div>
