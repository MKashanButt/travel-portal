                ...
                <div class="p-6 text-gray-900 dark:text-[#EDEDEC]">
                    <table class="min-w-full divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                        ...
                        <tbody class="bg-white dark:bg-[#161615] divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                            @foreach ($pccs as $pcc)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $pcc->code }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $pcc->user->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $pcc->agentSales->count() }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                                        <a href="{{ route('pccs.show', $pcc) }}"
                                        ...
                                        <form action="{{ route('pccs.destroy', $pcc) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
