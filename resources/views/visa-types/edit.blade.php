        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div
                ...
                <div class="p-6 text-gray-900 dark:text-[#EDEDEC]">
                    <form method="POST" action="{{ route('visa-types.update', $visaType) }}" class="space-y-6 max-w-xl">
                        @csrf
                        @method('PUT')

                        <!-- Type -->
                        <div>
                            <x-input-label for="types" :value="__('Visa Type')" />
                            <x-text-input id="types" name="types" type="text" class="mt-1 block w-full"
                                :value="old('types', $visaType->types)" required autofocus />
                            <x-input-error :messages="$errors->get('types')" class="mt-2" />
                            <p class="mt-1 text-sm text-[#706f6c] dark:text-[#A1A09A]">
                                {{ __('Enter the visa type (e.g., Tourist, Business, Student)') }}
                            </p>
                        </div>
