<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <a href="{{ route('visa-types.create') }}" 
                    ...
                    <table class="min-w-full divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                        <thead class="bg-[#FDFDFC] dark:bg-[#161615]">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">Sales Count</th>
                        ...
                        <tbody class="bg-white dark:bg-[#161615] divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                            @foreach($visaTypes as $visaType)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $visaType->types }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $visaType->agentSales->count() }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                                        <a href="{{ route('visa-types.show', $visaType) }}" 
                                           class="text-[#f53003] dark:text-[#FF4433] hover:underline underline-offset-4">View</a>
                                        <a href="{{ route('visa-types.edit', $visaType) }}" 
                                           class="text-[#f53003] dark:text-[#FF4433] hover:underline underline-offset-4">Edit</a>
                                        <form action="{{ route('visa-types.destroy', $visaType) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
