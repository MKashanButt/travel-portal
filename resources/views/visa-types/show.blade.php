        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div
                ...
                <div class="p-6 text-gray-900 dark:text-[#EDEDEC]">
                    <h3 class="text-lg font-medium mb-4">{{ __('Visa Type Details') }}</h3>
                    <dl class="grid grid-cols-1 gap-4">
                        <div>
                            <dt class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ __('Type') }}</dt>
                            <dd class="mt-1 text-sm">{{ $visaType->types }}</dd>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <dt class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ __('Created At') }}</dt>
                                <dd class="mt-1 text-sm">{{ $visaType->created_at->format('Y-m-d H:i:s') }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ __('Last Updated') }}</dt>
                                <dd class="mt-1 text-sm">{{ $visaType->updated_at->format('Y-m-d H:i:s') }}</dd>
                            </div>
                        </div>
                    </dl>
                </div>
            ...
            <div
                ...
                <div class="p-6 text-gray-900 dark:text-[#EDEDEC]">
                    ...
                    @if ($visaType->agentSales->count() > 0)
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                                ...
                                <tbody
                                    ...
                                    @foreach ($visaType->agentSales as $sale)
                                        <tr>
                    ...
                    @else
                        <p class="text-[#706f6c] dark:text-[#A1A09A] text-sm">
                            {{ __('No agent sales found for this visa type.') }}</p>
                    @endif
                </div>
            </div>
        </div>
